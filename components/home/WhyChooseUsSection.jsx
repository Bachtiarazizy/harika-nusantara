  ...
  return (
    <Section className="bg-coffee-light/5 py-16">
      <div className="max-w-6xl mx-auto px-2">
        {/* Title Section */}
        <motion.div
          className="flex flex-col items-start text-left md:items-center md:text-center mb-12"
          initial="hidden"
          whileInView="visible"
        ...
        >
          <motion.h2 className="subtitle" variants={{ hidden: { opacity: 0, y: 20 }, visible: { opacity: 1, y: 0 } }} transition={{ duration: 0.6 }}>
            WHY CHOOSE US
          </motion.h2>
          ...
          <motion.h1 className="text-3xl lg:text-4xl font-bold text-coffee-dark mb-4" variants={{ hidden: { opacity: 0, y: 30 }, visible: { opacity: 1, y: 0 } }} transition={{ duration: 0.6, delay: 0.1 }}>
            Rooted in Integrity,
            <br className="md:hidden" /> Grown with Purpose
          </motion.h1>
        ...
        >
          {cards.map((card, index) => (
            <motion.div
              key={index}
              className="bg-white p-6 rounded-lg shadow-md text-left md:text-center"
              variants={{
            ...
            >
              <div className="bg-coffee-light/10 w-16 h-16 mx-0 md:mx-auto rounded-full flex items-center justify-center mb-4">{card.icon}</div>
